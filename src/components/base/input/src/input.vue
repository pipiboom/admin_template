...
import Migrating from "../../../../../node_modules/element-ui/src/mixins/migrating";
import calcTextareaHeight from "./calcTextareaHeight";
import merge from "../../../../../node_modules/element-ui/src/utils/merge";
...
export default {
  ...
  methods: {
    ...
    handleInput(event) {
      if (this.type === 'textarea' || this.showWordLimit) {
      ...
      let len = str.length;
      let charCode = -1;
      input.value = event.target.value.replace(/^\s*|\s*$/g, "");
      for (let i = 0; i < len; i++) {
        ...
        if (realLength > this.$attrs.maxlength) {
          if (this.type === "number") {
            input.value = Number(str.substring(0, i));
            break;
          } else {
            input.value = str.substring(0, i);
            break;
          }
        }
      }
      if (this.isNumber) {
        input.value = this.onlyInt(event.target.value);
      } else if (this.isFloatNumber) {
        input.value = this.numberOnly(event.target.value);
      };
      this.$emit("input", event.target.value);
      this.$nextTick(this.setNativeInputValue);
    },
